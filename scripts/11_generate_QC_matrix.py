...
def parse_args():
    ...
    if len(sys.argv) <= 1:
        parser.print_help()
        sys.exit("Error: No input parameters provided. Please provide required parameters.")
...
def main():
    ...
    ordered_metrics.append(("%GC Total Raw Reads", raw_gc_avg))
    ordered_metrics.append(("Average Raw Read Length", int(round(avg_raw_read_length))))
    ordered_metrics.append(("Average Trimmed Read Length", int(round(avg_trimmed_read_length))))
    ordered_metrics.append(("Number of Trimmed Reads (TrimGalore)", int(round(total_trimmed_reads))))
    ordered_metrics.append(("Percentage of Trimmed Reads (TrimGalore)", round((total_trimmed_reads / total_raw_reads) * 100, 2) if total_raw_reads > 0 else 0))
    ...
    ordered_metrics.append(("Number of Clean Reads Mapped to Escherichia coli", int(round(ecoli_total_reads))))
    ordered_metrics.append(("Percentage of Clean Reads Mapped to Escherichia coli", pct_ecoli))
    ordered_metrics.append(("Number of Clean Reads Mapped to Mycoplasma", int(round(myco_total_reads))))
    ordered_metrics.append(("Percentage of Clean Reads Mapped to Mycoplasma", pct_myco))
    ordered_metrics.append(("Percentage of Trimmed Reads Mapped to rRNAs (RiboDetector)", perc_rRNA_ribodetector))

    # 3. STAR Metrics and UMI-deduped Strand Metrics
    ordered_metrics.append(("Number of UMI-deduped Input Reads (STAR)", umi_input_reads_star))
    ordered_metrics.append(("Percentage of UMI-deduped Reads (STAR)", perc_umi_reads_star))
    ordered_metrics.append(("Average Mapped UMI-deduped Fragment Length (STAR)", avg_umi_fragment_length_star))
    ordered_metrics.append(("Percentage of UMI-deduped Reads on the Forward Strand", umi_forward_strand))
    ordered_metrics.append(("Percentage of UMI-deduped Reads on the Reverse Strand", umi_reverse_strand))
    ordered_metrics.append(("Percentage of UMI-deduped Reads with Failed Strand", umi_failed_strand))
    ordered_metrics.append(("Number of UMI-deduped Reads Uniquely Mapped to Human (STAR)", umi_unique_reads_star))
    ordered_metrics.append(("Percentage of UMI-deduped Reads Uniquely Mapped to Human (STAR)", perc_umi_unique_reads_star))
    ordered_metrics.append(("Number of UMI-deduped Reads Multi-mapped to Human (STAR)", umi_multi_reads_star))
    ordered_metrics.append(("Percentage of UMI-deduped Reads Multi-mapped to Human (STAR)", perc_umi_multi_reads_star))
    ordered_metrics.append(("Number of Splices from UMI-deduped Reads Uniquely Mapped to Human (STAR)", umi_splices_star))
    ordered_metrics.append(("Number of Human Unmapped UMI-deduped Reads (STAR)", umi_unmapped_reads_star))
    ordered_metrics.append(("Number of Chimeric UMI-deduped Reads Mapped to Human (STAR)", umi_chimeric_reads_star))
    ordered_metrics.append(("Mismatch Rate per Base for UMI-deduped Reads (STAR)", umi_mismatch_rate_star))
    ...
    ordered_metrics.append(("Number of UMI-deduped Reads Mapped to Human Downstream (featureCounts)", fc_downstream))
    ...
    ordered_metrics.append(("Number of UMI-deduped Reads Mapped to Human Blacklist Regions (featureCounts)", fc_blacklist))
