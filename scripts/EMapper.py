...
Key modifications in this version:
  ...
  8. **Modified insert size filtering for paired-end reads**:
     For paired reads with mate mapped on the same reference, the effective insert size is calculated as the difference between
     the maximum end coordinate and the minimum start coordinate of all alignment blocks (as in featureCounts). 
     Only perform insert size filtering when both reads are non-spliced (i.e. have only one alignment block each).
     If either read is spliced (i.e. has >1 block), skip insert size filtering and directly merge the blocks.
...
def merge_pair_blocks(primary_blocks: List[Tuple[int, int]], paired_blocks: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """
    ...
    Example:
      primary_blocks = [(100,150), (200,250)]
      paired_blocks = [(350,400)]
      returns [(100,150), (200,250), (350,400)]

    Parameters:
      primary_blocks: Blocks for the first read.
      paired_blocks: Blocks for the mate.

    Returns:
      Merged non-overlapping intervals.
    """
    if not primary_blocks or not paired_blocks:
        _module_logger.warning("Empty blocks detected for read pair. Skipping merge.")
        return []
    all_blocks = primary_blocks + paired_blocks
...
def validate_coverage(coverage_dict: Dict[str, "ChunkedCoverage"], logger: logging.Logger = None) -> None:
    ...
    """
    for chromosome, coverage_object in coverage_dict.items():
        array_coverage = coverage_object.copy_array()
        negative_indices = np.where(array_coverage < 0)[0]
        if negative_indices.size > 0:
            if logger:
                logger.error(f"Negative coverage detected in {chromosome} at positions: {negative_indices}")
            raise ValueError(f"Negative coverage detected in {chromosome} at positions: {negative_indices}")
...
def gather_alignments(args: argparse.Namespace, logger: logging.Logger) -> Tuple[int, int, int, int, int, Dict[Any, List[Any]], Dict[str, int], List[Any]]:
    ...
    For paired-end reads, process as a pair by:
      ...
      - For reads with mate mapped on the same reference, compute the effective insert size using all alignment blocks:
            * Retrieve all alignment blocks from both primary and paired alignments.
            * Determine the minimum start coordinate and maximum end coordinate.
            * Effective insert size = (maximum end coordinate) - (minimum start coordinate).
      - Only perform insert size filtering if both reads are non-spliced (i.e. have exactly one block each).
        Otherwise, skip filtering and directly merge the alignment blocks.
      - Record strand information as ("paired", primary_is_reverse, paired_is_reverse).
    ...
    Returns:
      ...
      unique_fragment_records: list of unique mapped records.
         For paired, each record is (chrom, pre-merged intervals, ("paired", primary_is_reverse, paired_is_reverse));
    ...
    except (IOError, ValueError) as error_exception:
        logger.error(f"Error opening BAM file: {error_exception}")
        sys.exit(1)
    ...
    if not bam_file.references:
        ...
        for alignment in bam_file.fetch(until_eof=True):
            if alignment.is_unmapped or alignment.reference_id < 0:
                continue
            chromosome = getattr(alignment, "reference_name", f"chr{alignment.reference_id}")
            for (start, end) in alignment.get_blocks():
                ref_lengths[chromosome] = max(ref_lengths[chromosome], end)
        bam_file.reset()
        ref_lengths = {chromosome: length for chromosome, length in ref_lengths.items() if length > 0}
    else:
    ...
    def get_mate(primary_alignment: pysam.AlignedSegment, max_retries: int = 3) -> Any:
        """Retrieve mate using caching to reduce repeated I/O. Limits cache size."""
        query_name = primary_alignment.query_name
        if query_name in mate_cache:
            paired_alignment = mate_cache.pop(query_name)
            mate_cache[query_name] = paired_alignment
            return paired_alignment
        if len(mate_cache) >= MAX_MATE_CACHE_SIZE:
            mate_cache.popitem(last=False)
        paired_alignment_local = None
        for _ in range(max_retries):
            try:
                paired_alignment_local = bam_file.mate(primary_alignment)
                break
            except (ValueError, StopIteration):
                continue
        mate_cache[query_name] = paired_alignment_local
        return paired_alignment_local

    for primary_alignment in bam_file.fetch(until_eof=True):
        total_alignments += 1
        alignment_query_name = primary_alignment.query_name
        try:
            primary_alignment_blocks = primary_alignment.get_blocks()
        except Exception as error_exception:
            logger.error(f"Skipping read {alignment_query_name} due to error in get_blocks(): {error_exception}")
            continue
        if not primary_alignment_blocks:
            continue
        total_blocks += len(primary_alignment_blocks)
        if primary_alignment.is_unmapped or primary_alignment.reference_id < 0:
            continue
        chromosome = bam_file.get_reference_name(primary_alignment.reference_id) if bam_file.references else getattr(primary_alignment, "reference_name", f"chr{primary_alignment.reference_id}")
        ref_length = ref_lengths.get(chromosome, 0)
        try:
            merged_blocks = list(cached_merge_intervals(tuple(primary_alignment_blocks)))
        except ValueError as error_exception:
            logger.error(f"Skipping read {alignment_query_name} due to invalid blocks: {error_exception}")
            continue
        if primary_alignment.is_paired:
            if alignment_query_name in processed_pairs or not primary_alignment.is_read1:
                continue
            paired_alignment = get_mate(primary_alignment)
            if paired_alignment is None:
                logger.debug(f"Processing {alignment_query_name} as single-end due to mate retrieval failure.")
                if is_unique_alignment(primary_alignment, args):
                    unique_fragment_records.append((chromosome, merged_blocks, ("single", primary_alignment.is_read1, primary_alignment.is_reverse)))
                else:
                    alignment_score = float(primary_alignment.get_tag("AS")) if primary_alignment.has_tag("AS") else float(primary_alignment.mapping_quality)
                    multi_mapped_reads_dict[(alignment_query_name, False)].append((chromosome, merged_blocks, alignment_score, ("single", primary_alignment.is_read1, primary_alignment.is_reverse)))
                read_pair_status[(alignment_query_name, False)] = (1 if not primary_alignment.is_unmapped else 0, False)
                continue
            if paired_alignment.reference_id != primary_alignment.reference_id:
                logger.warning(f"Mate mapped to a different chromosome for {alignment_query_name}; treating reads separately.")
                if is_unique_alignment(primary_alignment, args):
                    unique_fragment_records.append((chromosome, list(cached_merge_intervals(tuple(primary_alignment.get_blocks()))), ("single", primary_alignment.is_read1, primary_alignment.is_reverse)))
                else:
                    alignment_score = float(primary_alignment.get_tag("AS")) if primary_alignment.has_tag("AS") else float(primary_alignment.mapping_quality)
                    multi_mapped_reads_dict[(alignment_query_name, False)].append((chromosome, list(cached_merge_intervals(tuple(primary_alignment.get_blocks()))), alignment_score, ("single", primary_alignment.is_read1, primary_alignment.is_reverse)))
                mate_chromosome = bam_file.get_reference_name(paired_alignment.reference_id) if bam_file.references else getattr(paired_alignment, "reference_name", f"chr{paired_alignment.reference_id}")
                if (not paired_alignment.is_unmapped) and is_unique_alignment(paired_alignment, args):
                    unique_fragment_records.append((mate_chromosome, list(cached_merge_intervals(tuple(paired_alignment.get_blocks()))), ("single", paired_alignment.is_read1, paired_alignment.is_reverse)))
                elif paired_alignment and not paired_alignment.is_unmapped:
                    mate_alignment_score = float(paired_alignment.get_tag("AS")) if paired_alignment.has_tag("AS") else float(paired_alignment.mapping_quality)
                    multi_mapped_reads_dict[(alignment_query_name, True)].append((mate_chromosome, list(cached_merge_intervals(tuple(paired_alignment.get_blocks()))), mate_alignment_score, ("single", paired_alignment.is_read1, paired_alignment.is_reverse)))
                read_pair_status[(alignment_query_name, True)] = (1, True)
                continue

            # For paired reads mapped on the same reference, check for spliced alignments.
            # Retrieve alignment blocks for both primary and paired alignments.
            if (not primary_alignment.mate_is_unmapped) and (primary_alignment.reference_id == primary_alignment.next_reference_id):
                primary_alignment_blocks = primary_alignment.get_blocks()
                paired_alignment_blocks = paired_alignment.get_blocks()
                # Check if both reads are non-spliced (only one alignment block each)
                if len(primary_alignment_blocks) == 1 and len(paired_alignment_blocks) == 1:
                    # Both reads are non-spliced, perform effective insert size filtering.
                    combined_alignment_blocks = primary_alignment_blocks + paired_alignment_blocks
                    minimum_start_coordinate = min(block[0] for block in combined_alignment_blocks)
                    maximum_end_coordinate = max(block[1] for block in combined_alignment_blocks)
                    effective_insert_size = maximum_end_coordinate - minimum_start_coordinate
                    # Filter out the pair if the effective insert size exceeds the maximum allowed insert size.
                    if args.max_insert_size >= 0 and effective_insert_size > args.max_insert_size:
                        continue
                    # Merge the alignment blocks for further processing.
                    merged_blocks = merge_intervals(combined_alignment_blocks)
                else:
                    # At least one read is spliced (more than one alignment block), so skip insert size filtering.
                    merged_blocks = merge_intervals(primary_alignment_blocks + paired_alignment_blocks)
            else:
                merged_blocks = merge_pair_blocks(primary_alignment.get_blocks(), paired_alignment.get_blocks())

            if not merged_blocks:
                continue
            total_blocks += len(merged_blocks)
            read_pair_status[(alignment_query_name, True)] = (2, True)
            strand_info = ("paired", primary_alignment.is_reverse, paired_alignment.is_reverse)
            if is_unique_alignment(primary_alignment, args) and is_unique_alignment(paired_alignment, args):
                unique_fragment_records.append((chromosome, merged_blocks, strand_info))
            else:
                alignment_score = ((float(primary_alignment.get_tag("AS")) + float(paired_alignment.get_tag("AS"))) / 2.0
                                   if primary_alignment.has_tag("AS") and paired_alignment.has_tag("AS")
                                   else (float(primary_alignment.mapping_quality) + float(paired_alignment.mapping_quality)) / 2.0)
                multi_mapped_reads_dict[(alignment_query_name, True)].append((chromosome, merged_blocks, alignment_score, strand_info))
            processed_pairs.add(alignment_query_name)
        else:
            read_pair_status[(alignment_query_name, False)] = (1 if not primary_alignment.is_unmapped else 0, False)
            if is_unique_alignment(primary_alignment, args):
                unique_fragment_records.append((chromosome, merged_blocks, ("single", primary_alignment.is_read1, primary_alignment.is_reverse)))
...
def is_unique_alignment(alignment: pysam.AlignedSegment, args: argparse.Namespace) -> bool:
    """
    ...
    if args.bam_aligner == 'STAR':
        return (alignment.mapping_quality == 255)
    else:
        return (alignment.mapping_quality >= 30)
...
def add_unique_coverage(unique_coverage_dict: Dict[str, ChunkedCoverage],
    ...
    for record in unique_fragment_records:
        chromosome, merged_blocks, strand_info = record
        if chromosome not in unique_coverage_dict:
            logger.warning(f"Chromosome {chromosome} not found in reference lengths; skipping intervals.")
            continue
        for start, end in merged_blocks:
            if start < end:
                unique_coverage_dict[chromosome].add_one_coverage(start, end)
...
def update_fractions_for_chunk(read_keys: List[Any],
    ...
    for key in read_keys:
        ...
        weight_list = []
        for (chromosome, merged_blocks, align_score, strand_info) in alignments:
            interval_sum = sum(unique_coverage_dict[chromosome].sum_range(start, end) +
                               multi_coverage_prev[chromosome].sum_range(start, end)
                               for start, end in merged_blocks)
...
def build_multi_coverage_unstranded(fraction_dict: Dict[Any, List[float]],
    ...
    """
    multi_coverage = {}
    for chromosome, coverage_object in unique_coverage_dict.items():
        multi_coverage[chromosome] = ChunkedCoverage(coverage_object.total_length)
    for key, frac_list in fraction_dict.items():
        alignments = multi_mapped_reads_dict[key]
        for i, (chromosome, merged_blocks, align_score, strand_info) in enumerate(alignments):
            if chromosome not in multi_coverage:
                _module_logger.warning(f"Chromosome {chromosome} not in unique_coverage_dict; skipping candidate.")
                continue
            ...
            for start, end in merged_blocks:
                if start < end:
                    multi_coverage[chromosome].add_fractional_coverage(start, end, fraction_value)
...
def build_multi_coverage_stranded(fraction_dict: Dict[Any, List[float]],
    ...
    coverage_f1r2 = {}
    coverage_f2r1 = {}
    for chromosome, length in ref_lengths.items():
        coverage_f1r2[chromosome] = ChunkedCoverage(length)
        coverage_f2r1[chromosome] = ChunkedCoverage(length)
    for key, frac_list in fraction_dict.items():
        alignments = multi_mapped_reads_dict[key]
        for i, (chromosome, merged_blocks, align_score, strand_info) in enumerate(alignments):
            if chromosome not in coverage_f1r2 or chromosome not in coverage_f2r1:
                _module_logger.warning(f"Chromosome {chromosome} missing in ref_lengths; skipping candidate.")
                continue
            fraction_value = frac_list[i]
            if fraction_value <= 0:
                continue
            if isinstance(strand_info, tuple):
                ...
                if marker == "paired":
                    _, primary_is_reverse, paired_is_reverse = strand_info
                    is_f1r2 = (not primary_is_reverse) and paired_is_reverse
                elif marker == "single":
            ...
            for start, end in merged_blocks:
                if start < end:
                    if is_f1r2:
                        coverage_f1r2[chromosome].add_fractional_coverage(start, end, fraction_value)
                    else:
                        coverage_f2r1[chromosome].add_fractional_coverage(start, end, fraction_value)
...
def run_em_unstranded(multi_mapped_reads_dict: Dict[Any, List[Any]],
    ...
    coverage_all = {}
    for chromosome, coverage_object in unique_coverage_dict.items():
        multi_coverage_prev[chromosome] = ChunkedCoverage(coverage_object.total_length)
        coverage_all[chromosome] = ChunkedCoverage(coverage_object.total_length)
    ...
    for chromosome in unique_coverage_dict:
        multi_coverage_prev[chromosome].chunks = initial_multi_coverage[chromosome].copy_chunks()
        num_chunks = min(len(coverage_all[chromosome].chunks), len(unique_coverage_dict[chromosome].chunks),
                         len(multi_coverage_prev[chromosome].chunks))
        for idx in range(num_chunks):
            target = coverage_all[chromosome].chunks[idx]
            np.copyto(target, unique_coverage_dict[chromosome].chunks[idx])
            np.add(target, multi_coverage_prev[chromosome].chunks[idx], out=target)
    for chromosome in initial_multi_coverage:
        initial_multi_coverage[chromosome].free_memory()
    initial_multi_coverage.clear()

    # Create a reusable multi_coverage dictionary.
    reusable_multi_coverage = {chromosome: ChunkedCoverage(unique_coverage_dict[chromosome].total_length)
                               for chromosome in unique_coverage_dict}
    ...
    for chromosome in multi_coverage_prev:
        multi_coverage_prev[chromosome].free_memory()
    multi_coverage_prev.clear()
    # Instead of directly updating with warmup objects, perform a deep copy.
    for chromosome, warm_obj in warmup_multi_coverage.items():
        new_cov = ChunkedCoverage(warm_obj.total_length)
        new_cov.copy_chunks_from(warm_obj)
        multi_coverage_prev[chromosome] = new_cov
    for chromosome in warmup_multi_coverage:
        warmup_multi_coverage[chromosome].free_memory()
    warmup_multi_coverage.clear()
    for chromosome in coverage_all:
        num_chunks = min(len(coverage_all[chromosome].chunks), len(unique_coverage_dict[chromosome].chunks),
                         len(multi_coverage_prev[chromosome].chunks))
        for idx in range(num_chunks):
            target = coverage_all[chromosome].chunks[idx]
    ...
    for iteration in range(1, max_iter + 1):
        ...
        for chromosome in reusable_multi_coverage:
            # Reset the reusable object: clear existing arrays (fill with zeros) and update by copying new chunks.
            for arr in reusable_multi_coverage[chromosome].chunks:
                arr.fill(0)
            reusable_multi_coverage[chromosome].copy_chunks_from(multi_coverage_current[chromosome])
        for chromosome in multi_coverage_prev:
            multi_coverage_prev[chromosome].free_memory()
        multi_coverage_prev.clear()
        multi_coverage_prev.update({chromosome: reusable_multi_coverage[chromosome] for chromosome in reusable_multi_coverage})
        for chromosome in multi_coverage_current:
            multi_coverage_current[chromosome].free_memory()
...
def parted_coverage_unique_final(ref_lengths: Dict[str, int], unique_fragment_records: List[Any]) -> Tuple[Dict[str, ChunkedCoverage], Dict[str, ChunkedCoverage]]:
    ...
    For paired fragments, the stored marker is ("paired", primary_is_reverse, paired_is_reverse).
    ...
    for chromosome, length in ref_lengths.items():
        coverage_f1r2_unique[chromosome] = ChunkedCoverage(length)
        coverage_f2r1_unique[chromosome] = ChunkedCoverage(length)
    for record in unique_fragment_records:
        chromosome, merged_blocks, strand_info = record
        if isinstance(strand_info, tuple) and strand_info[0] == "paired":
            _, primary_is_reverse, paired_is_reverse = strand_info
            if (not primary_is_reverse) and paired_is_reverse:
                for start, end in merged_blocks:
                    if start < end:
                        coverage_f1r2_unique[chromosome].add_one_coverage(start, end)
            else:
                for start, end in merged_blocks:
                    if start < end:
                        coverage_f2r1_unique[chromosome].add_one_coverage(start, end)
...
def parted_coverage_multi_final(fraction_dict: Dict[Any, List[float]],
    ...
    coverage_f2r1_multi = {}
    for chromosome, length in ref_lengths.items():
        coverage_f1r2_multi[chromosome] = ChunkedCoverage(length)
        coverage_f2r1_multi[chromosome] = ChunkedCoverage(length)
    for key, frac_list in fraction_dict.items():
        alignments = multi_mapped_reads_dict[key]
        for i, candidate in enumerate(alignments):
            chromosome, merged_blocks, align_score, strand_info = candidate
            fraction_value = frac_list[i]
            if fraction_value <= 0:
                continue
            if isinstance(strand_info, tuple) and strand_info[0] == "paired":
            ...
            for start, end in merged_blocks:
                if start < end:
                    if is_f1r2:
                        coverage_f1r2_multi[chromosome].add_fractional_coverage(start, end, fraction_value)
                    else:
                        coverage_f2r1_multi[chromosome].add_fractional_coverage(start, end, fraction_value)
...
def generate_bedgraph_lines(coverage_dict: Dict[str, ChunkedCoverage]) -> List[str]:
    ...
    """
    lines = []
    for chromosome in sorted(coverage_dict.keys()):
        if not coverage_dict[chromosome].chunks:
            _module_logger.warning(f"No coverage data found for chromosome {chromosome}.")
            continue
        pos_offset = 0
        for chunk in coverage_dict[chromosome].chunks:
            ...
            for idx in change_indices:
                if current_pos < idx:
                    ...
                    if val != 0:
                        lines.append(f"{chromosome}\t{pos_offset + current_pos}\t{pos_offset + idx}\t{val:.4f}\n")
                current_pos = idx
            ...
            if current_pos < len(chunk):
                ...
                if val != 0:
                    ...
                    lines.append(f"{chromosome}\t{pos_offset + current_pos}\t{end_pos}\t{val:.4f}\n")
...
def write_bedgraph(coverage_dict: Dict[str, ChunkedCoverage], out_path: str, logger: logging.Logger) -> None:
    ...
    except IOError as error_exception:
        logger.error(f"IOError writing bedGraph file {out_path}: {error_exception}")
        raise
    except Exception as error_exception:
        logger.error(f"Unexpected error writing bedGraph file {out_path}: {error_exception}")
        raise
...
def main():
    ...
    try:
        ...
        if args.mode == "uniq":
            unique_coverage_dict = {chromosome: ChunkedCoverage(length) for chromosome, length in ref_lengths.items() if length > 0}
        ...
        if args.split_by_strand == "no":
            ...
            cov_f1r2_total = {}
            cov_f2r1_total = {}
            for chromosome in cov_f1r2_unique:
                cov_f1r2_total[chromosome] = ChunkedCoverage(cov_f1r2_unique[chromosome].total_length)
                cov_f2r1_total[chromosome] = ChunkedCoverage(cov_f2r1_unique[chromosome].total_length)
                num_chunks = min(len(cov_f1r2_total[chromosome].chunks),
                                 len(cov_f1r2_unique[chromosome].chunks),
                                 len(cov_f1r2_multi[chromosome].chunks))
                for i in range(num_chunks):
                    target = cov_f1r2_total[chromosome].chunks[i]
                    np.copyto(target, cov_f1r2_unique[chromosome].chunks[i])
                    np.add(target, cov_f1r2_multi[chromosome].chunks[i], out=target)
                    target = cov_f2r1_total[chromosome].chunks[i]
                    np.copyto(target, cov_f2r1_unique[chromosome].chunks[i])
                    np.add(target, cov_f2r1_multi[chromosome].chunks[i], out=target)
        ...
        logger.exception(f"Error in main: {error_exception}")
        sys.exit(1)
    finally:
