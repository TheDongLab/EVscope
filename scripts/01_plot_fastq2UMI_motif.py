...
# Set SVG output to use text as vector and set font to Arial globally
...
def main():
    ...
    try:
        ...
        ax.set_title(args.head, fontsize=10)
        plt.tight_layout()

        # Save motif logo in three formats: PNG, PDF, SVG
        ...
        svg_path = os.path.join(args.output_dir_qc, f"{args.head}_{args.num_bases}BP_motif_logo.svg")
        plt.savefig(png_path, dpi=300)
        plt.savefig(pdf_path, dpi=300)
        plt.savefig(svg_path, dpi=300)
        plt.close()
        logging.info(f"Motif logo saved as: {png_path}, {pdf_path}, and {svg_path}")
    except Exception as e:
