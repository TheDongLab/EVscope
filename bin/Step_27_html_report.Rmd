...
.main-container {
  ...
  margin-left: calc(var(--toc-width) + 50px) !important; /* 自动计算间距 */
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (toc) {
    ...
    btn.innerHTML = '<span>☰</span> Close Menu'; 
...
display_all_images <- function(subdir_path, pattern, main_caption) {
  ...
  if (length(img_files) > 0) {
    cat(paste0('<div style="margin-top: 1.5rem;"><strong style="font-size: 1.2rem; color: #1a5276;">📈 ', main_caption, '</strong></div>\n\n'))
...
if (file.exists(img_path)) {
    ...
    cat('<div style="padding: 1rem; background: #e8f4fc; border-radius: 8px; text-align: center; color: #1a5276;">ℹ Pipeline diagram not available</div>\n\n')
}
...
if (check_step_dir("Step_01_Raw_QC")) {
  display_iframe("Step_01_Raw_QC", "_R1_001_fastqc\\.html$", "FastQC Report — Read 1")
  display_iframe("Step_01_Raw_QC", "_R2_001_fastqc\\.html$", "FastQC Report — Read 2")
}
...
if (check_step_dir("Step_04_Trimmed_QC")) {
  ...
  display_iframe("Step_04_Trimmed_QC", "_R2_clean_fastqc\\.html$", "Trimmed FastQC Report — Read 2")
}
...
if (check_step_dir("Step_07_Strand_Detection")) {
  ...
  # --- Splice/kb: DNA contamination metric ---
  tsv_files <- list.files(
    file.path(sample_dir, "Step_07_Strand_Detection"),
  ...
  if (length(tsv_files) > 0) {
    ...
    if (!is.null(strand_df) && "Splice_per_kb" %in% colnames(strand_df)) {
      spkb_val <- strand_df[["Splice_per_kb"]][1]
      cat(paste0(
        "<div style='border-left:4px solid #2196F3; padding:10px 16px; margin:12px 0;",
        " background:#f0f7ff; border-radius:4px;'>",
        "<b>Splices per kilobase (splice/kb): ", spkb_val, "</b><br/>",
        "<span style='color:#555; font-size:0.92em;'>",
        "DNA contamination metric (splice junctions per kb of mapped sequence). ",
        "Reference: DNase-treated ~1.67 | No DNase ~0.13 (EV RNA-seq pilot, N=2). ",
        "Low splice/kb indicates residual gDNA contamination. ",
        "EVscope applies gDNA correction (opposite-strand subtraction); ",
        "splice/kb is an independent complementary QC check.",
        "</span></div>"
      ))
    }
  }
...
if (check_step_dir("Step_25_EMapper_BigWig_Quantification")) {
  ...
  cat('<strong style="color: #1e8449;">✓ BigWig Files Generated</strong><br/>')
